...
updatelocation = Blueprint('updatelocation', __name__)
...
@updatelocation.route('/updatelocation', methods=['GET', 'POST'])
@login_required
def updatelocation_index():
    ...
    if request.method == 'POST':
        ...
        #     pass
        try:
            userinfo.lon = my_json['lon']
            userinfo.lat = my_json['lat']
        except:
            pass
        db.session.commit()
        ...
        try:
            return {'lon': userinfo.lon,'lat':userinfo.lat}
        except:
            return "error"